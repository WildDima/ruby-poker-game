  ...
  let(:players) do
    ...
    context 'winner is first player' do
      ...
      it 'does return first player as winner' do
        expect(subject.winner_player).to eq(players[0])
        expect(subject.winner_hand.rank).to eq('Three of a kind')
      end
    end
    ...
    context 'winner is second player' do
      ...
      it 'does return second player as winner' do
        expect(subject.winner_player).to eq(players[1])

        expect(subject.winner_hand.rank).to eq('Four of a kind')
      end
    end
    ...
    context 'winner is third player' do
      ...
      it 'does return second player as winner' do
        expect(subject.winner_player).to eq(players[2])
        expect(subject.winner_hand.rank).to eq('Flush')
      end
    end
end